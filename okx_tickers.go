package marketdata

import (
	"errors"
	"fmt"
	"sync/atomic"
	"time"

	"github.com/Hongssd/myokxapi"
	"github.com/shopspring/decimal"
)

type OkxTickers struct {
	parent          *OkxMarketData
	perConnSubNum   int64
	perSubMaxLen    int
	Exchange        Exchange
	TickerMap       *MySyncMap[string, *Ticker]
	WsClientListMap *MySyncMap[*myokxapi.PublicWsStreamClient, *int64]
	WsClientMap     *MySyncMap[string, *myokxapi.PublicWsStreamClient]
	SubMap          *MySyncMap[string, *myokxapi.Subscription[myokxapi.WsTickers]]
	CallBackMap     *MySyncMap[string, func(ticker *Ticker, err error)]
}

func (om *OkxMarketData) newOkxTickers(config OkxTickersConfig) *OkxTickers {
	if config.PerConnSubNum == 0 {
		config.PerConnSubNum = 100
	}
	if config.PerSubMaxLen == 0 {
		config.PerSubMaxLen = 1000
	}
	o := &OkxTickers{
		parent:          om,
		perConnSubNum:   config.PerConnSubNum,
		perSubMaxLen:    config.PerSubMaxLen,
		Exchange:        OKX,
		TickerMap:       GetPointer(NewMySyncMap[string, *Ticker]()),
		WsClientListMap: GetPointer(NewMySyncMap[*myokxapi.PublicWsStreamClient, *int64]()),
		WsClientMap:     GetPointer(NewMySyncMap[string, *myokxapi.PublicWsStreamClient]()),
		SubMap:          GetPointer(NewMySyncMap[string, *myokxapi.Subscription[myokxapi.WsTickers]]()),
		CallBackMap:     GetPointer(NewMySyncMap[string, func(ticker *Ticker, err error)]()),
	}
	return o
}

func (o *OkxTickers) GetLastTicker(symbol string) (*Ticker, error) {
	ticker, ok := o.TickerMap.Load(symbol)
	if !ok {
		err := fmt.Errorf("symbol:%s ticker not found", symbol)
		return nil, err
	}
	return ticker, nil
}

func (o *OkxTickers) GetCurrentOrNewWsClient() (*myokxapi.PublicWsStreamClient, error) {
	return o.parent.GetPublicCurrentOrNewWsClient(o.perConnSubNum, o.WsClientListMap)
}

func (o *OkxTickers) SubscribeTicker(symbol string) error {
	return o.SubscribeTickerWithCallBack(symbol, nil)
}

func (o *OkxTickers) SubscribeTickers(symbols []string) error {
	return o.SubscribeTickersWithCallBack(symbols, nil)
}

func (o *OkxTickers) SubscribeTickerWithCallBack(symbol string, callback func(ticker *Ticker, err error)) error {
	return o.SubscribeTickersWithCallBack([]string{symbol}, callback)
}

func (o *OkxTickers) SubscribeTickersWithCallBack(symbols []string, callback func(ticker *Ticker, err error)) error {
	log.Infof("开始订阅Tickers，交易对数:%d, 总订阅数:%d", len(symbols), len(symbols))
	//订阅总数超过LEN次，分批订阅
	LEN := o.perSubMaxLen
	if len(symbols) > LEN {
		for i := 0; i < len(symbols); i += LEN {
			end := i + LEN
			if end > len(symbols) {
				end = len(symbols)
			}
			tempSymbols := symbols[i:end]
			client, err := o.GetCurrentOrNewWsClient()
			if err != nil {
				return err
			}
			err = o.subscribeOkxTickersMultiple(client, tempSymbols, callback)
			if err != nil {
				return err
			}
			currentCount, ok := o.WsClientListMap.Load(client)
			if !ok {
				return errors.New("WsClientListMap Load error")
			}
			log.Infof("Ticker分批订阅成功，此次订阅交易对:%v, 总数:%d，当前链接总订阅数:%d, 等待1秒后继续订阅...", tempSymbols, len(tempSymbols), *currentCount)
			time.Sleep(1000 * time.Millisecond)
		}
	} else {
		client, err := o.GetCurrentOrNewWsClient()
		if err != nil {
			return err
		}
		err = o.subscribeOkxTickersMultiple(client, symbols, callback)
		if err != nil {
			return err
		}
	}

	log.Infof("Ticker订阅结束，交易对数:%d, 总订阅数:%d", len(symbols), len(symbols))

	return nil
}

// 订阅OKX Tickers
func (o *OkxTickers) subscribeOkxTickersMultiple(okxWsClient *myokxapi.PublicWsStreamClient, symbols []string, callback func(ticker *Ticker, err error)) error {

	okxSub, err := okxWsClient.SubscribeTickersMultiple(symbols)
	if err != nil {
		log.Error(err)
		return err
	}
	for _, symbol := range symbols {
		symbolKey := symbol
		o.WsClientMap.Store(symbolKey, okxWsClient)
		o.SubMap.Store(symbolKey, okxSub)
		o.CallBackMap.Store(symbolKey, callback)
	}
	go func() {
		for {
			select {
			case err := <-okxSub.ErrChan():
				log.Error(err)
				if callback != nil {
					callback(nil, err)
				}
			case result := <-okxSub.ResultChan():
				Symbol := result.InstId

				if okx_common == nil {
					okx_common = (&okxCommon{}).InitCommon()
				}
				accountType := okx_common.GetAccountTypeFromSymbol(result.InstId)

				now := time.Now().UnixMilli()
				targetTs := stringToInt64(result.Ts) + o.parent.serverTimeDelta
				if targetTs > now {
					targetTs = now
				}

				//保存至Ticker
				var price24hPercent float64
				lastPrice, _ := decimal.NewFromString(result.Last)
				open24h, _ := decimal.NewFromString(result.Open24h)
				if !open24h.IsZero() {
					// 计算24小时价格变化百分比: (last - open24h) / open24h * 100
					priceChange := lastPrice.Sub(open24h)
					price24hPercent, _ = priceChange.Div(open24h).Mul(decimal.NewFromInt(100)).Float64()
				}

				ticker := &Ticker{
					Timestamp:       targetTs,
					Exchange:        o.Exchange.String(),
					AccountType:     accountType,
					Symbol:          result.InstId,
					LastPrice:       stringToFloat64(result.Last),
					Price24hPercent: price24hPercent,
					PrevPrice24h:    stringToFloat64(result.Open24h),
					HighPrice24h:    stringToFloat64(result.High24h),
					LowPrice24h:     stringToFloat64(result.Low24h),
					Volume24h:       stringToFloat64(result.Vol24h),
					Turnover24h:     stringToFloat64(result.VolCcy24h),
				}
				o.TickerMap.Store(Symbol, ticker)
				if callback != nil {
					callback(ticker, nil)
				}
			case <-okxSub.CloseChan():
				log.Info("订阅已关闭: ", okxSub.Args)
				return
			}
		}
	}()

	currentCount := int64(len(symbols))
	count, ok := o.WsClientListMap.Load(okxWsClient)
	if !ok {
		initCount := int64(0)
		count = &initCount
		o.WsClientListMap.Store(okxWsClient, &initCount)
	}
	atomic.AddInt64(count, currentCount)
	return nil
}

func (o *OkxTickers) Close() {
	o.WsClientListMap.Range(func(k *myokxapi.PublicWsStreamClient, v *int64) bool {
		err := k.Close()
		if err != nil {
			log.Error(err)
		}
		return true
	})

	o.TickerMap.Clear()
	o.WsClientListMap.Clear()
	o.WsClientMap.Clear()
	o.SubMap.Clear()
	o.CallBackMap.Clear()
}
