...
type XcoinTickersConfig struct {
	PerConnSubNum int64 //每条链接订阅的数量
	PerSubMaxLen  int   //每条链接每次订阅的最大数量
}

type BitgetTickersConfig struct {
	SpotConfig        BitgetTickersConfigBase // 现货
	UsdtFuturesConfig BitgetTickersConfigBase // USDT 合约
	CoinFuturesConfig BitgetTickersConfigBase // 币本位合约
	UsdcFuturesConfig BitgetTickersConfigBase // USDC 合约
}

type BitgetTickersConfigBase struct {
	PerConnSubNum int64 //每条链接订阅的数量
	PerSubMaxLen  int   //每条链接每次订阅的最大数量
}
