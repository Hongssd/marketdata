...
type AsterOrderBook struct {
	parent          *AsterMarketData
	SpotOrderBook   *asterOrderBookBase
	FutureOrderBook *asterOrderBookBase
}

type asterOrderBookBase struct {
	parent                    *AsterOrderBook
	...
	OrderBookCacheMap         *MySyncMap[string, *MySyncMap[int64, *myasterapi.WsDepth]]
	...
	WsClientMap               *MySyncMap[string, *myasterapi.WsStreamClient]                   //symbol->wsClient
...
// 根据类型获取基础
func (b *AsterOrderBook) getBaseMapFromAccountType(accountType AsterAccountType) (*asterOrderBookBase, error) {
	switch accountType {
	case ASTER_SPOT:
...
// 新建币安深度基础
func (b *AsterOrderBook) newAsterOrderBookBase(config AsterOrderBookConfigBase) *asterOrderBookBase {
	...
	return &asterOrderBookBase{
		...
		OrderBookCacheMap:         GetPointer(NewMySyncMap[string, *MySyncMap[int64, *myasterapi.WsDepth]]()),
...
// 获取当前或新建ws客户端
func (b *AsterOrderBook) GetCurrentOrNewWsClient(accountType AsterAccountType) (*myasterapi.WsStreamClient, error) {
	switch accountType {
	case ASTER_SPOT:
...
// 封装好的获取深度方法
func (b *AsterOrderBook) GetDepth(AsterAccountType AsterAccountType, symbol string, level int, timeoutMilli int64) (*Depth, error) {
...
// 封装好的获取深度方法(高性能视图模式)
func (b *AsterOrderBook) ViewDepth(AsterAccountType AsterAccountType, symbol string, level int, timeoutMilli int64, bizLogic func(*Depth) error) error {
	bmap, err := b.getBaseMapFromAccountType(AsterAccountType)
	if err != nil {
		return err
	}
...
func (b *asterOrderBookBase) subscribeAsterDepthMultipleWithZeroCopy(asterWsClient *myasterapi.WsStreamClient, symbols []string, callback func(depth *Depth, err error), isZeroCopy bool) error {
	...
	for _, symbol := range symbols {
		b.WsClientMap.Store(symbol, asterWsClient)
		b.SubMap.Store(symbol, asterSub)
	...
	go func() {
		for {
			...
			case result := <-asterSub.ResultChan():
				...
				//检测深度是否初始化
				_, err := b.checkAsterDepthIsReady(Symbol)
				if err != nil {
					//直接存入深度缓存
					b.saveAsterDepthCache(result)
					continue
				}

				//判断是否丢包
				if lastLowerU, ok := b.OrderBookLastUpdateIdMap.Load(Symbol); ok {
					if b.AccountType == ASTER_SPOT {
						if result.UpperU > lastLowerU+1 {
							...
							//重新初始化深度
							go func() {
								err := b.initAsterDepthFunc(result.Symbol)
								if err != nil {
									log.Error(err)
								}
							}()
				...
				//保存至OrderBook
				err = b.saveAsterDepthOrderBook(result)
				if err != nil {
					log.Error(err)
					continue
				}
	...
	for _, symbol := range symbols {
		...
		g.Go(func() error {
			//初始化深度池
			err = b.initAsterDepthFunc(symbol)
			if err != nil {
				log.Error(err)
				return err
			}
...
//// 取消订阅币安深度
//func (b *asterOrderBookBase) UnSubscribeAsterDepth(Symbol string) error {
//	asterSub, ok := b.SubMap.Load(Symbol)
//	if !ok {
//		return nil
//	}
...
//
//// 重新订阅币安深度
//func (b *asterOrderBookBase) ReSubscribeAsterDepth(Symbol string) error {
//	err := b.UnSubscribeAsterDepth(Symbol)
//	if err != nil {
//		return err
//	}
//	asterWsClient, ok := b.WsClientMap.Load(Symbol)
//	if !ok {
//		err := fmt.Errorf("symbol:%s asterWsClient not found", Symbol)
//		return err
//	}
...
// 初始化币安深度
func (b *asterOrderBookBase) initAsterDepthFunc(symbol string) error {
	...
	err := b.initAsterDepthOrderBook(symbol)
	for err != nil {
		// log.Error(err)
	...
	return b.saveAsterDepthOrderBookFromCache(symbol)
}

// 检测深度是否准备好
func (b *asterOrderBookBase) checkAsterDepthIsReady(Symbol string) (int64, error) {
...
// 初始化深度
func (b *asterOrderBookBase) initAsterDepthOrderBook(Symbol string) error {
	...
	case ASTER_SPOT:
		//重新初始化
		depth, err := aster.NewSpotRestClient("", "").NewSpotDepth().Symbol(Symbol).Limit(b.initOrderBookSize).Do()
		if err != nil {
			log.Error(err)
			return err
		}
	...
	case ASTER_FUTURE:
		//重新初始化
		depth, err := aster.NewFutureRestClient("", "").NewFutureDepth().Symbol(Symbol).Limit(50).Do()
		if err != nil {
			log.Error(err)
			return err
		}
...
func (b *asterOrderBookBase) saveAsterDepthOrderBookFromCache(Symbol string) error {

	// readyId, err := b.checkAsterDepthIsReady(Symbol)
	// if err != nil {
	// 	log.Error(err)
	// 	return err
	// }
	...
	if !ok {
		newMap := NewMySyncMap[int64, *myasterapi.WsDepth]()
		cacheMap = &newMap
		b.OrderBookCacheMap.Store(Symbol, cacheMap)
	}

	//按照LowerU排序
	var cacheList []myasterapi.WsDepth
	cacheMap.Range(func(k int64, v *myasterapi.WsDepth) bool {
		cacheList = append(cacheList, *v)
		return true
	})
	sort.Sort(SortAsterWsDepthSlice(cacheList))

	// log.Info(lastUpdateId)
	// log.Info(len(cacheList))
	targetCacheList := []myasterapi.WsDepth{}
	for index, v := range cacheList {
		if b.AccountType != ASTER_SPOT {
			if v.UpperU <= lastUpdateId && v.LowerU >= lastUpdateId {
				err := b.saveAsterDepthOrderBook(v)
				if err != nil {
					log.Error(err)
					return err
				}
...
// 将Depth缓存
func (b *asterOrderBookBase) saveAsterDepthCache(result myasterapi.WsDepth) {
	Symbol := result.Symbol
...
// 将Depth保存至OrderBook
func (b *asterOrderBookBase) saveAsterDepthOrderBook(result myasterapi.WsDepth) error {
	Symbol := result.Symbol
...
func (b *asterOrderBookBase) GetUidAndPreUid(result myasterapi.WsDepth) (int64, int64) {
	UId := int64(0)
...
// 订阅深度
func (b *AsterOrderBook) SubscribeOrderBook(accountType AsterAccountType, symbol string) error {
	return b.SubscribeOrderBookWithCallBack(accountType, symbol, nil)
}

// 批量订阅深度
func (b *AsterOrderBook) SubscribeOrderBooks(accountType AsterAccountType, symbols []string) error {
	return b.SubscribeOrderBooksWithCallBack(accountType, symbols, nil)
}

// 订阅深度并带上回调
func (b *AsterOrderBook) SubscribeOrderBookWithCallBack(accountType AsterAccountType, symbol string, callback func(depth *Depth, err error)) error {
...
func (b *AsterOrderBook) SubscribeOrderBooksWithCallBackAndZeroCopy(accountType AsterAccountType, symbols []string, callback func(depth *Depth, err error), isZeroCopy bool) error {
	...
	switch accountType {
	case ASTER_SPOT:
		currentAsterOrderBookBase = b.SpotOrderBook
	case ASTER_FUTURE:
		currentAsterOrderBookBase = b.FutureOrderBook
	default:
		return ErrorAccountType
	}
	//订阅总数超过LEN次，分批订阅
	LEN := currentAsterOrderBookBase.perSubMaxLen
	if len(symbols) > LEN {
		for i := 0; i < len(symbols); i += LEN {
			...
			currentCount, ok := currentAsterOrderBookBase.WsClientListMap.Load(client)
			if !ok {
...
func (b *asterOrderBookBase) Close() {
